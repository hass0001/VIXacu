...
#if 1
void HTTPServer_Handler(uint8_t seqnum)
{
	uint8_t s ;
	uint16_t len;
	uint32_t gettime = 0;
...
#else
__NO_RETURN void HTTPServer_Thread(void* argument)
{
 	uint8_t seqnum = _SOCK_HTTPS;
	uint8_t s ;
	uint16_t len;
	uint32_t gettime = 0;
...
__NO_RETURN void HTTPServer_Thread(void* argument)
{
	...
	{
#if 0
		eventFlag = osEventFlagsWait(Spi1RxCallback_EvtId, 0x00000001U, osFlagsWaitAny, osWaitForever);

		if(eventFlag == osFlagsError || \
			eventFlag == osFlagsErrorUnknown || \
				eventFlag == osFlagsErrorTimeout || \
					eventFlag == osFlagsErrorResource || \
						eventFlag == osFlagsErrorParameter || \
							eventFlag == osFlagsErrorISR) {
			Dprintf("Spi1RxCallback_EvtId Error!\r\n");
		}
		else
		{
#endif

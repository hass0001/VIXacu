...
/* USER CODE BEGIN 0 */
#include "RTC_Proc.h"
uint16_t t1msTim3_485;
uint16_t t50msTim6_Net;
uint16_t t50msTim6_Tcp;

volatile uint32_t delay_ticks = 0;
extern void PCALED_TIM2HandlerProc(void);
extern uint8_t UHSAliveCheckTimer;
extern uint8_t gGoVect;
extern uint8_t flag_sent_http_request;
extern void RTC_ApplyToDayF( TIME *myDayTime);
extern void INT_TIM2HandlerProc(void);
...
//	BaseType_t xHigherPriorityTaskWoken = pdFALSE;
	...
	case (uint32_t)TIM1:	/* RTC 1ms */
		RTC_ApplyToDayF( &toDayTime);
		break;
	case (uint32_t)TIM2:	/* door long Length */
		INT_TIM2HandlerProc();
		break;
...
//		}
	...
	case (uint32_t)TIM5:	/* PCALED Run  */
		...
		break;
	case (uint32_t)TIM6:	/* Network Response timeout */
			t50msTim6_Net++;
			t50msTim6_Tcp++;
		break;
	case (uint32_t)TIM7:	/* My Delay timer */
	    if (delay_ticks > 0) {
			delay_ticks--;
		}
		break;
